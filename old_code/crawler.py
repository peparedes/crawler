...
def ljquery_flat ():
	curl = '/bin/curl.exe -d mode=login -d user=kenghao -d password=smallcat1003 http://www.livejournal.com/interface/flat'
...
def ljquery_xmlrpc ():
	...
	xml = appendParamMember(xml, 'username', 'kenghao', 'string'); 
	xml = appendParamMember(xml, 'password', 'smallcat1003', 'string'); 
	xml = endXmlStr(xml);
